...
- 所有滚动边缘渐变毛玻璃必须复用 `Components/TransparentGradientBlur.swift`，使用满强度 `systemChromeMaterial` 实时背景模糊和线性渐变 mask。修改名片夹或搜索页时，必须同时检查名片夹底部、最近添加顶部、搜索页底部以及输入态 top=433 的覆盖位置。
